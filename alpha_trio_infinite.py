# alpha_trio_infinite.py (FULL, GitHub Actions safe: auto-pick model via ListModels)
...
HTTP_TIMEOUT = 30
UA_HEADERS = {
    "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AlphaTrioBot/3.0 (+https://github.com/)",
...
# -------------------------
# Gemini: ListModels 기반 자동 선택
# -------------------------
def pick_gemini_generate_model() -> str:
    url = f"https://generativelanguage.googleapis.com/v1beta/models?key={GEMINI_KEY}"
    r = requests.get(url, timeout=HTTP_TIMEOUT)
    if r.status_code != 200:
        raise Exception(f"ListModels 실패: {r.status_code} {r.text}")
    ...
    if not candidates:
        raise Exception(f"generateContent 지원 모델 없음. ListModels 응답: {json.dumps(j)[:1000]}")
    ...
    chosen = candidates[0]
    print("🧠 Gemini model chosen:", chosen)
    return chosen
