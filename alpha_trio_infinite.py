...
def send_to_discord(agent_id, title, content):
    ...
    # 디스코드 전송 (중요: data= 사용, json= 금지)
    requests.post(DISCORD_URL, data={"content": formatted_msg}, timeout=30)
    time.sleep(1.5)
...
if __name__ == "__main__":
    try:
        ...
        # [프롬프트: JSON 생성 금지 및 마크다운 강제 명령]
        prompt = f"""당신은 Alpha-Trio 위원회입니다. 시장 분가가로서 전문적인 보고서를 작성하십시오.

        시장 데이터: {m_data} | 과거 기억: {memory}
        사용자 질문: {USER_QUESTION if USER_QUESTION else '정기 시장 해부'}

        [엄격한 보고 지침 - 위반 시 시스템 오류 발생]
        1. **JSON, 코드 블록, 백틱(`), 대괄호([]), 중괄호({})를 절대 사용하지 마십시오.** 순수 텍스트와 마크다운 문법만 허용됩니다.
        2. 에이전트 A, B, C의 분석을 다음 구분자로 정확히 나누십시오:
           - Agent A 시작: [AGENT_A]
           - Agent B 시작: [AGENT_B]
           - Agent C 시작: [AGENT_C]
        3. 굵은 글씨(**), 불렛 포인트(*), 제목(##), 구분선(---)만 사용하여 전문 보고서를 작성하십시오.
        4. 각 에이전트 특성:
           - A(Quant): $RSI$, $BB$, $TDI$, $ATR$ 등 기술적 지표로 과학적 분석
           - B(Dewey): 투자 철학과 실천 전략 제시
           - C(Beckett): 날카로운 리스크 비판과 실존적 위험 경고

        [AGENT_A]
        여기에 Agent A의 분석을 작성하십시오...

        [AGENT_B]
        여기에 Agent B의 분석을 작성하십시오...

        [AGENT_C]
        여기에 Agent C의 분석을 작성하십시오...
        """
