...
type UniverseStock = Pick<LiveQuote, "symbol" | "name" | "market" | "price" | "change" | "volume" | "marketStatus" | "tradedAt"> & {
  ...
  bollingerSignalType: string | null;
  bollingerEma20: number | null;
  ...
  bbMacdBreakout: boolean;
  bbMacdSignalType: string | null;
  bbMacdValue: number | null;
  bbMacdAverage9: number | null;
  bbMacdStandardDeviation9: number | null;
  bbMacdUpperBand: number | null;
  bbMacdLowerBand: number | null;
  bbMacdZScore: number | null;
  bbMacdTradingDate: string | null;
  macdSignalBreakout: boolean;
  macdSignalType: string | null;
  ...
  zlemaSignal: boolean;
  zlemaSignalType: string | null;
  zlema5: number | null;
  zlema20: number | null;
  zlemaRsi30: number | null;
  zlemaGoldenCrossDate: string | null;
  zlemaRsiBreakoutDate: string | null;
  zlemaTradingDate: string | null;
  variableRegression60m: boolean;
  variableRegressionSignalType: string | null;
  variableRegressionValue: number | null;
  variableRegressionPreviousValue: number | null;
  variableRegressionDistancePercent: number | null;
  variableRegressionTradingDateTime: string | null;
  grahamDodd: boolean;
...
const pipelineLibrary: PipelineNode[] = [
  ...
  { id: "zlema", name: "ZLEMA", category: "ZLEMA", detail: "ZLEMA(5)가 ZLEMA(20)를 상향 돌파하고 RSI(30)가 70을 상향 돌파한 시점이 각각 오늘 또는 어제인 종목을 찾습니다.", rule: "(GC 오늘 OR 어제) AND (RSI(30) 70 돌파 오늘 OR 어제)" },
  { id: "variableRegression60m", name: "변동회귀선60분봉", category: "60MIN", detail: "시가총액 3,000억원 이상 종목의 60분봉에서 이중 50봉 선형회귀로 변동회귀선 VL을 계산하고, 종가가 VL 위에서 VL이 처음 상승 전환한 종목을 찾습니다.", rule: "시총 ≥ 3,000억원 · 종가 > VL · VL[0] > VL[1] ≤ VL[2]" },
...
function matchesNode(nodeId: string, stock: UniverseStock, stats: UniverseStats) {
  ...
  if (nodeId === "zlema") return stock.zlemaSignal;
  if (nodeId === "variableRegression60m") return stock.marketCap != null && stock.marketCap >= 300_000_000_000 && stock.variableRegression60m;
...
function buildIntersectionResults(items: UniverseStock[], nodes: PipelineNode[], stats: UniverseStats) {
  ...
  } else if (nodes.some((node) => node.id === "variableRegression60m")) {
    results.sort((left, right) => (
      (left.variableRegressionDistancePercent ?? Number.POSITIVE_INFINITY)
      - (right.variableRegressionDistancePercent ?? Number.POSITIVE_INFINITY)
    ));
  } else if (nodes.some((node) => node.id === "zlema")) {
    results.sort((left, right) => (
      (right.zlemaRsi30 ?? Number.NEGATIVE_INFINITY)
      - (left.zlemaRsi30 ?? Number.NEGATIVE_INFINITY)
    ));
  ...
  } else if (nodes.some((node) => node.id === "bbMacd")) {
    results.sort((left, right) => (
      (left.bbMacdZScore ?? Number.POSITIVE_INFINITY)
      - (right.bbMacdZScore ?? Number.POSITIVE_INFINITY)
    ));
  } else if (nodes.some((node) => node.id === "multiEnvelope")) {
    results.sort((left, right) => (
  ...
  } else if (nodes.some((node) => node.id === "elephantCandle")) {
    results.sort((left, right) => (
...
export default function Home() {
  ...
  const [bbMacdLoading, setBbMacdLoading] = useState(false);
  const [bbMacdDataReady, setBbMacdDataReady] = useState(false);
  const [bbMacdProgress, setBbMacdProgress] = useState({ completed: 0, total: 0 });
  ...
  const [zlemaLoading, setZlemaLoading] = useState(false);
  const [zlemaDataReady, setZlemaDataReady] = useState(false);
  const [zlemaProgress, setZlemaProgress] = useState({ completed: 0, total: 0 });
  const [variableRegressionLoading, setVariableRegressionLoading] = useState(false);
  const [variableRegressionDataReady, setVariableRegressionDataReady] = useState(false);
  const [variableRegressionProgress, setVariableRegressionProgress] = useState({ completed: 0, total: 0 });
  ...
  const nodeCandidateLabel = (node: PipelineNode) => {
    ...
    if (node.id === "zlema" && !zlemaDataReady) return zlemaLoading ? `ZLEMA 계산 ${zlemaProgress.completed}/${zlemaProgress.total}` : "ZLEMA·RSI 새로받기 필요";
    if (node.id === "variableRegression60m" && !variableRegressionDataReady) return variableRegressionLoading ? `60분봉 계산 ${variableRegressionProgress.completed}/${variableRegressionProgress.total}` : "60분봉 회귀선 새로받기 필요";
  ...
  const fetchUniverse = useCallback(async () => {
    ...
    try {
      ...
      const normalizedStocks = payload.stocks.map((stock) => ({
        ...
        bbMacdBreakout: false,
        bbMacdSignalType: null,
        bbMacdValue: null,
        bbMacdAverage9: null,
        bbMacdStandardDeviation9: null,
        bbMacdUpperBand: null,
        bbMacdLowerBand: null,
        bbMacdZScore: null,
        bbMacdTradingDate: null,
        macdSignalBreakout: false,
        macdSignalType: null,
        macdValue: null,
        macdSignal9: null,
        macdHistogram: null,
        ...
        zlemaSignal: false,
        zlemaSignalType: null,
        zlema5: null,
        zlema20: null,
        zlemaRsi30: null,
        zlemaGoldenCrossDate: null,
        zlemaRsiBreakoutDate: null,
        zlemaTradingDate: null,
        variableRegression60m: false,
        variableRegressionSignalType: null,
        variableRegressionValue: null,
        variableRegressionPreviousValue: null,
        variableRegressionDistancePercent: null,
        variableRegressionTradingDateTime: null,
        grahamDodd: false,
      ...
      setZlemaDataReady(false);
      setVariableRegressionDataReady(false);
  ...
  const fetchBollingerSqueezes = useCallback(async (items: UniverseStock[]) => {
    const batchSize = 10;
    const batches = Array.from({ length: Math.ceil(items.length / batchSize) }, (_, index) => items.slice(index * batchSize, (index + 1) * batchSize));
    const matches = new Map<string, {
      signalType: string;
    ...
    setBollingerProgress({ completed: 0, total: batches.length });
    try {
      const worker = async () => {
        while (nextBatch < batches.length) {
          const batch = batches[nextBatch];
          nextBatch += 1;
          try {
            const response = await fetch(`/api/bollinger-squeeze?symbols=${batch.map((item) => item.symbol).join(",")}`, { cache: "no-store" });
            const payload = await response.json() as {
              evaluated?: number;
              results?: Array<{
                symbol: string;
            ...
            completed += 1;
            setBollingerProgress({ completed, total: batches.length });
          }
        }
      };
      await Promise.all(Array.from({ length: Math.min(3, batches.length) }, () => worker()));
      if (evaluated === 0) throw new Error("볼린저스퀴즈 계산에 필요한 BB·EMA20 일봉 데이터가 없습니다.");
      const enriched = items.map((item) => {
        const match = matches.get(item.symbol);
        return {
          ...item,
          bollingerSqueeze: Boolean(match),
          bollingerSignalType: match?.signalType ?? null,
          bollingerEma20: match?.ema20 ?? null,
          bollingerBandwidth: match?.bandwidth ?? null,
          bollingerBandwidthAverage40: match?.bandwidthAverage40 ?? null,
          bollingerSqueezeRatio: match?.squeezeRatio ?? null,
          bollingerTradingDate: match?.tradingDate ?? null,
        };
      });
      setBollingerDataReady(true);
      setUniverseStocks(enriched);
      return enriched;
    } finally {
  ...
  const fetchBbMacdBreakouts = useCallback(async (items: UniverseStock[]) => {
    const batchSize = 10;
    const batches = Array.from({ length: Math.ceil(items.length / batchSize) }, (_, index) => items.slice(index * batchSize, (index + 1) * batchSize));
    const matches = new Map<string, {
      signalType: string;
    ...
    let completed = 0;
    let evaluated = 0;
    setBbMacdLoading(true);
    setBbMacdProgress({ completed: 0, total: batches.length });
    try {
      const worker = async () => {
        while (nextBatch < batches.length) {
          const batch = batches[nextBatch];
          nextBatch += 1;
          try {
            const response = await fetch(`/api/bb-macd?symbols=${batch.map((item) => item.symbol).join(",")}`, { cache: "no-store" });
            const payload = await response.json() as {
              evaluated?: number;
              results?: Array<{
                symbol: string;
            ...
            completed += 1;
            setBbMacdProgress({ completed, total: batches.length });
          }
        }
      };
      await Promise.all(Array.from({ length: Math.min(3, batches.length) }, () => worker()));
      if (evaluated === 0) throw new Error("BB on MACD 계산에 필요한 EMA·표준편차 일봉 데이터가 없습니다.");
      const enriched = items.map((item) => {
        const match = matches.get(item.symbol);
        return {
          ...item,
          bbMacdBreakout: Boolean(match),
          bbMacdSignalType: match?.signalType ?? null,
          bbMacdValue: match?.bbMacd ?? null,
          bbMacdAverage9: match?.average9 ?? null,
          bbMacdStandardDeviation9: match?.standardDeviation9 ?? null,
          bbMacdUpperBand: match?.upperBand ?? null,
          bbMacdLowerBand: match?.lowerBand ?? null,
          bbMacdZScore: match?.zScore ?? null,
          bbMacdTradingDate: match?.tradingDate ?? null,
        };
      });
      setBbMacdDataReady(true);
      setUniverseStocks(enriched);
      return enriched;
    } finally {
      setBbMacdLoading(false);
    }
  }, []);

  const fetchMacdSignalBreakouts = useCallback(async (items: UniverseStock[]) => {
    const batchSize = 10;
    const batches = Array.from({ length: Math.ceil(items.length / batchSize) }, (_, index) => items.slice(index * batchSize, (index + 1) * batchSize));
    const matches = new Map<string, {
      signalType: string;
      ...
      signal9: number;
      histogram: number;
      signalGapPercent: number;
      tradingDate: string;
    }>();
    let nextBatch = 0;
    let completed = 0;
    let evaluated = 0;
    setMacdSignalLoading(true);
    setMacdSignalProgress({ completed: 0, total: batches.length });
    try {
      const worker = async () => {
        while (nextBatch < batches.length) {
          const batch = batches[nextBatch];
          nextBatch += 1;
          try {
            const response = await fetch(`/api/macd-signal?symbols=${batch.map((item) => item.symbol).join(",")}`, { cache: "no-store" });
            const payload = await response.json() as {
              evaluated?: number;
              results?: Array<{
                symbol: string;
            ...
            completed += 1;
            setMacdSignalProgress({ completed, total: batches.length });
          }
        }
      };
      await Promise.all(Array.from({ length: Math.min(3, batches.length) }, () => worker()));
      if (evaluated === 0) throw new Error("MACD 계산에 필요한 EMA(12·26)와 시그널 EMA(9) 일봉 데이터가 없습니다.");
      const enriched = items.map((item) => {
        const match = matches.get(item.symbol);
        return {
          ...item,
          macdSignalBreakout: Boolean(match),
          macdSignalType: match?.signalType ?? null,
          macdValue: match?.macd ?? null,
          macdSignal9: match?.signal9 ?? null,
          macdHistogram: match?.histogram ?? null,
          macdSignalGapPercent: match?.signalGapPercent ?? null,
          macdTradingDate: match?.tradingDate ?? null,
        };
      });
      setMacdSignalDataReady(true);
      setUniverseStocks(enriched);
      return enriched;
    } finally {
      setMacdSignalLoading(false);
    }
  }, []);

  const fetchZlemaSignals = useCallback(async (items: UniverseStock[]) => {
    const batchSize = 10;
    const batches = Array.from({ length: Math.ceil(items.length / batchSize) }, (_, index) => items.slice(index * batchSize, (index + 1) * batchSize));
    const matches = new Map<string, {
      signalType: string;
      ...
      rsiBreakoutDate: string;
      tradingDate: string;
    }>();
    let nextBatch = 0;
    let completed = 0;
    let evaluated = 0;
    setZlemaLoading(true);
    setZlemaProgress({ completed: 0, total: batches.length });
    try {
      const worker = async () => {
        while (nextBatch < batches.length) {
          const batch = batches[nextBatch];
          nextBatch += 1;
          try {
            const response = await fetch(`/api/zlema?symbols=${batch.map((item) => item.symbol).join(",")}`, { cache: "no-store" });
            const payload = await response.json() as {
              evaluated?: number;
              results?: Array<{
                symbol: string;
            ...
            completed += 1;
            setZlemaProgress({ completed, total: batches.length });
          }
        }
      };
      await Promise.all(Array.from({ length: Math.min(3, batches.length) }, () => worker()));
      if (evaluated === 0) throw new Error("ZLEMA(5·20)와 RSI(30) 계산에 필요한 일봉 데이터가 없습니다.");
      const enriched = items.map((item) => {
        const match = matches.get(item.symbol);
        return {
          ...item,
          zlemaSignal: Boolean(match),
          zlemaSignalType: match?.signalType ?? null,
          zlema5: match?.zlema5 ?? null,
          zlema20: match?.zlema20 ?? null,
          zlemaRsi30: match?.rsi30 ?? null,
          zlemaGoldenCrossDate: match?.goldenCrossDate ?? null,
          zlemaRsiBreakoutDate: match?.rsiBreakoutDate ?? null,
          zlemaTradingDate: match?.tradingDate ?? null,
        };
      });
      setZlemaDataReady(true);
      setUniverseStocks(enriched);
      return enriched;
    } finally {
      setZlemaLoading(false);
    }
  }, []);

  const fetchVariableRegression60m = useCallback(async (items: UniverseStock[]) => {
    const eligible = items.filter((item) => item.marketCap != null && item.marketCap >= 300_000_000_000);
    const batchSize = 10;
    ...
    let completed = 0;
    let evaluated = 0;
    setVariableRegressionLoading(true);
    setVariableRegressionProgress({ completed: 0, total: batches.length });
    try {
      const worker = async () => {
        while (nextBatch < batches.length) {
          const batch = batches[nextBatch];
          nextBatch += 1;
          try {
            const tickers = batch.map((item) => `${item.symbol}.${item.market === "KOSDAQ" ? "KQ" : "KS"}`);
            const response = await fetch(`/api/variable-regression-60m?tickers=${tickers.join(",")}`, { cache: "no-store" });
            const payload = await response.json() as {
              evaluated?: number;
              results?: Array<{
                symbol: string;
            ...
            completed += 1;
            setVariableRegressionProgress({ completed, total: batches.length });
          }
        }
      };
      await Promise.all(Array.from({ length: Math.min(3, batches.length) }, () => worker()));
      if (eligible.length > 0 && evaluated === 0) throw new Error("변동회귀선 계산에 필요한 60분봉 데이터가 없습니다.");
      const enriched = items.map((item) => {
        const match = matches.get(item.symbol);
        return {
          ...item,
          variableRegression60m: Boolean(match),
          variableRegressionSignalType: match?.signalType ?? null,
          variableRegressionValue: match?.variableLine ?? null,
          variableRegressionPreviousValue: match?.previousVariableLine ?? null,
          variableRegressionDistancePercent: match?.distancePercent ?? null,
          variableRegressionTradingDateTime: match?.tradingDateTime ?? null,
        };
      });
      setVariableRegressionDataReady(true);
      setUniverseStocks(enriched);
      return enriched;
    } finally {
      setVariableRegressionLoading(false);
    }
  }, []);
  ...
  const runWorkflow = async () => {
    ...
    try {
      ...
      if (selectedConditionPipeline.some((node) => node.id === "bbMacd") && !bbMacdDataReady) {
        universe = await fetchBbMacdBreakouts(universe);
      }
      ...
      if (selectedConditionPipeline.some((node) => node.id === "zlema") && !zlemaDataReady) {
        universe = await fetchZlemaSignals(universe);
      }
      if (selectedConditionPipeline.some((node) => node.id === "variableRegression60m") && !variableRegressionDataReady) {
        universe = await fetchVariableRegression60m(universe);
      }
  ...
  const refreshWorkflowData = async () => {
    ...
    try {
      ...
      const zlemaUniverse = await fetchZlemaSignals(macdUniverse);
      const regressionUniverse = await fetchVariableRegression60m(zlemaUniverse);
      const universe = await fetchGrahamDoddCandidates(regressionUniverse);
      const currentStats = buildUniverseStats(universe);
    ...
    <div className="site-shell">
      ...
      {view === "workflow" && (
        <main className="page-content subpage">
          ...
          <section className="workflow-layout">
            ...
            <aside className="bento-card node-inspector">
              ...
              <dl>
                ...
                <div><dt>후보 풀</dt><dd>{(selectedNode.id === "weekly10" && !weeklyDataReady) || (selectedNode.id === "consensusUp" && !consensusDataReady) || (selectedNode.id === "candleResistance" && !candleResistanceDataReady) || (selectedNode.id === "weeklyBodyReversal" && !weeklyBodyDataReady) || (selectedNode.id === "elephantCandle" && !elephantDataReady) || (selectedNode.id === "bollingerSqueeze" && !bollingerDataReady) || (selectedNode.id === "multiEnvelope" && !multiEnvelopeDataReady) || (selectedNode.id === "bbMacd" && !bbMacdDataReady) || (selectedNode.id === "macdSignal" && !macdSignalDataReady) || (selectedNode.id === "zlema" && !zlemaDataReady) || (selectedNode.id === "variableRegression60m" && !variableRegressionDataReady) || (selectedNode.id === "grahamDodd" && !grahamDoddDataReady) ? "새로받기 또는 검색 필요" : universeStocks.length ? `${nodeCandidateCounts[selectedNode.id].toLocaleString("ko-KR")}개` : "가져오기 전"}</dd></div>
                ...
                {selectedNode.id === "bollingerSqueeze" && <div><dt>스퀴즈 기준</dt><dd>BB(20,2) 폭 ÷ 40일 평균 ≤ 70%</dd></div>}
                ...
                {selectedNode.id === "bbMacd" && <div><dt>MACD 산식</dt><dd>EMA(8) - EMA(26)</dd></div>}
                {selectedNode.id === "bbMacd" && <div><dt>밴드 산식</dt><dd>EMA(9) ± 표준편차(9) × 0.8</dd></div>}
                {selectedNode.id === "bbMacd" && <div><dt>구간 조건</dt><dd>금일 또는 전일 BBMacd &lt; 0</dd></div>}
                {selectedNode.id === "bbMacd" && <div><dt>결과 순서</dt><dd>상단밴드 초과 Z값 낮은 순</dd></div>}
                {selectedNode.id === "macdSignal" && <div><dt>MACD 산식</dt><dd>EMA(12) - EMA(26)</dd></div>}
                ...
                {selectedNode.id === "zlema" && <div><dt>ZLEMA(5)</dt><dd>EMA(C + (C-C[2]), 5)</dd></div>}
                {selectedNode.id === "zlema" && <div><dt>ZLEMA(20)</dt><dd>EMA(C + (C-C[9]), 20)</dd></div>}
                {selectedNode.id === "zlema" && <div><dt>RSI 조건</dt><dd>Wilder RSI(30) CrossUp 70</dd></div>}
                {selectedNode.id === "zlema" && <div><dt>발생 범위</dt><dd>각 신호가 오늘 또는 어제</dd></div>}
                {selectedNode.id === "variableRegression60m" && <div><dt>차트 주기</dt><dd>60분봉 · 최근 1개월</dd></div>}
                {selectedNode.id === "variableRegression60m" && <div><dt>변동회귀선</dt><dd>A + (A - A1) · 각 LRV 50봉</dd></div>}
                {selectedNode.id === "variableRegression60m" && <div><dt>첫 상승 변곡</dt><dd>VL[0] &gt; VL[1] ≤ VL[2]</dd></div>}
                {selectedNode.id === "variableRegression60m" && <div><dt>시총 기준</dt><dd>3,000억원 이상</dd></div>}
                ...
                {intersectionResults.map((result) => (
                  <button type="button" key={result.symbol} className="intersection-result" onClick={() => analyzeWorkflowStock(result)} aria-label={`${result.name} 종목분석 열기`}>
                    ...
                    <div className="result-data-tags">
                      ...
                      {result.bollingerSqueeze && <>
                        <span className="market-cap">{result.bollingerSignalType}</span>
                        <span className="market-cap">스퀴즈 {result.bollingerSqueezeRatio?.toFixed(1)}%</span>
                        <span className="market-cap">EMA20 {result.bollingerEma20 == null ? "-" : won(Math.round(result.bollingerEma20))}</span>
                        <span className="market-cap">밴드폭 {result.bollingerBandwidth?.toFixed(2)}</span>
                      ...
                      {result.bbMacdBreakout && <>
                        <span className="market-cap">{result.bbMacdSignalType}</span>
                        <span className="market-cap">BBMacd {result.bbMacdValue?.toFixed(2)}</span>
                        <span className="market-cap">상단밴드 {result.bbMacdUpperBand?.toFixed(2)}</span>
                        <span className="market-cap">중심 EMA(9) {result.bbMacdAverage9?.toFixed(2)}</span>
                        <span className="market-cap">표준편차 {result.bbMacdStandardDeviation9?.toFixed(2)}</span>
                        <span className="market-cap">Z값 {result.bbMacdZScore?.toFixed(2)}</span>
                        {result.bbMacdTradingDate && <span className="market-cap">신호일 {candleDateLabel(result.bbMacdTradingDate)}</span>}
                      </>}
                      {result.macdSignalBreakout && <>
                        <span className="market-cap">{result.macdSignalType}</span>
                        <span className="market-cap">MACD {result.macdValue?.toFixed(2)}</span>
                        <span className="market-cap">Signal(9) {result.macdSignal9?.toFixed(2)}</span>
                        <span className="market-cap">히스토그램 +{result.macdHistogram?.toFixed(2)}</span>
                      ...
                      {result.zlemaSignal && <>
                        <span className="market-cap">{result.zlemaSignalType}</span>
                        <span className="market-cap">ZLEMA(5) {result.zlema5?.toFixed(2)}</span>
                        <span className="market-cap">ZLEMA(20) {result.zlema20?.toFixed(2)}</span>
                        <span className="market-cap">RSI(30) {result.zlemaRsi30?.toFixed(2)}</span>
                        {result.zlemaGoldenCrossDate && <span className="market-cap">GC {candleDateLabel(result.zlemaGoldenCrossDate)}</span>}
                        {result.zlemaRsiBreakoutDate && <span className="market-cap">RSI 돌파 {candleDateLabel(result.zlemaRsiBreakoutDate)}</span>}
                      </>}
                      {result.variableRegression60m && <>
                        <span className="market-cap">{result.variableRegressionSignalType}</span>
                        <span className="market-cap">VL {result.variableRegressionValue == null ? "-" : won(Math.round(result.variableRegressionValue))}</span>
                        <span className="market-cap">종가-VL +{result.variableRegressionDistancePercent?.toFixed(3)}%</span>
                        {result.variableRegressionTradingDateTime && <span className="market-cap">60분봉 {new Date(result.variableRegressionTradingDateTime).toLocaleString("ko-KR", { timeZone: "Asia/Seoul", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit" })}</span>}
                      </>}
                      {result.grahamDodd && <>
          ...
          <p className="disclaimer">종목 가져오기 노드는 NAVER 금융의 KOSPI·KOSDAQ 상장 종목에서 ETF·ETN을 제외하고 최종 시가총액 3,000억원 이상만 불러옵니다. 캔들볼륨 저항선돌파 노드는 최근 90봉 음봉의 가격·거래량 저항선과 Wilder ADX(11)를 실제 일봉으로 계산합니다. 주봉 3주 음봉후 첫 양봉 노드는 최근 52주 고점 대비 20% 이상 하락, 직전 3주 음봉 몸통 연속 확대, 금주 첫 양봉을 실제 주봉으로 계산합니다. 코끼리캔들 노드는 첨부식의 기본 SearchMode 1에 따라 몸통비율 70% 이상, 몸통이 전일 Wilder ATR(100)의 1.3배 이상, SMA(8) 방향 상승인 양봉을 실제 일봉으로 계산합니다. 볼린저스퀴즈 노드는 BB(20,2) 밴드폭이 40일 평균의 70% 이하인 수축 상태에서 EMA20 상향돌파 또는 저가 EMA20 ±0.3% 터치가 나온 양봉을 실제 일봉으로 계산합니다. 멀티엔벨로프 노드는 첨부식의 기본값인 일봉 RMA(20), DailyWidthFactor 7, SignalMode 1을 적용해 금일 또는 전일 저가가 하단선을 이탈한 뒤 종가가 하단선을 상향 돌파한 종목을 계산합니다. BB on MACD 노드는 EMA(8)-EMA(26) 값에 EMA(9) ± 표준편차(9) × 0.8 밴드를 적용해 금일 또는 전일 MACD가 음수인 상태에서 상단밴드를 처음 돌파한 종목을 계산합니다. MACD 노드는 EMA(12)-EMA(26) 값이 0 이하인 상태에서 EMA(9) 시그널선을 처음 상향 돌파한 종목을 계산합니다. ZLEMA 노드는 ZLEMA(5)의 ZLEMA(20) 골든크로스와 Wilder RSI(30)의 70 상향돌파가 각각 오늘 또는 어제 발생한 종목을 실제 일봉으로 계산합니다. 변동회귀선60분봉 노드는 Yahoo Finance 60분봉에서 A와 A1을 각각 50봉 선형회귀로 계산하고, 시가총액 3,000억원 이상이면서 종가가 VL 위에 있고 VL이 처음 상승 전환한 종목을 표시합니다. 그레이엄도드지수 노드는 FnGuide 연간 컨센서스에서 2027년 예상 PER과 2026년 예상 PBR이 모두 양수인 종목만 평가해 두 값의 곱이 22.55 이하인 후보를 표시합니다. 컨센서스상향 노드는 로컬 증권리포트의 최근 5거래일 목표가상향 파일에서 종목명·코드만 추출한 공개용 스냅샷을 사용합니다. 모든 조건 노드는 이 전체 유니버스를 대상으로 계산하며, 선택한 조건의 교집합만 표시합니다. 투자 자문이 아닙니다.</p>
        </main>
      )}
