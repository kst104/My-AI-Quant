...
function buildIntersectionResults(items: UniverseStock[], nodes: PipelineNode[], stats: UniverseStats) {
  ...
  if (nodes.some((node) => node.id === "weeklyBodyReversal")) {
...
export default function Home() {
  ...
  const refreshWorkflowData = async () => {
    ...
    try {
      ...
      const universe = await fetchWeeklyBodyReversals(resistanceUniverse);
      const currentStats = buildUniverseStats(universe);
    ...
    <div className="site-shell">
      ...
      {view === "workflow" && (
        <main className="page-content subpage">
          ...
          <section className="workflow-layout">
            ...
            <aside className="bento-card node-inspector">
              ...
              <dl>
                ...
                <div><dt>후보 풀</dt><dd>{(selectedNode.id === "weekly10" && !weeklyDataReady) || (selectedNode.id === "consensusUp" && !consensusDataReady) || (selectedNode.id === "candleResistance" && !candleResistanceDataReady) || (selectedNode.id === "weeklyBodyReversal" && !weeklyBodyDataReady) ? "새로받기 또는 검색 필요" : universeStocks.length ? `${nodeCandidateCounts[selectedNode.id].toLocaleString("ko-KR")}개` : "가져오기 전"}</dd></div>
          ...
          <p className="disclaimer">종목 가져오기 노드는 NAVER 금융의 KOSPI·KOSDAQ 상장 종목에서 ETF·ETN을 제외하고 최종 시가총액 3,000억원 이상만 불러옵니다. 캔들볼륨 저항선돌파 노드는 최근 90봉 음봉의 가격·거래량 저항선과 Wilder ADX(11)를 실제 일봉으로 계산합니다. 주봉 3주 음봉후 첫 양봉 노드는 최근 52주 고점 대비 20% 이상 하락, 직전 3주 음봉 몸통 연속 확대, 금주 첫 양봉을 실제 주봉으로 계산합니다. 컨센서스상향 노드는 로컬 증권리포트의 최근 5거래일 목표가상향 파일에서 종목명·코드만 추출한 공개용 스냅샷을 사용합니다. 모든 조건 노드는 이 전체 유니버스를 대상으로 계산하며, 선택한 조건의 교집합만 표시합니다. 투자 자문이 아닙니다.</p>
        </main>
      )}
