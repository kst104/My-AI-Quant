...
function buildIntersectionResults(items: UniverseStock[], nodes: PipelineNode[], stats: UniverseStats) {
  ...
  if (nodes.some((node) => node.id === "candleResistance")) {
...
export default function Home() {
  ...
  const refreshWorkflowData = async () => {
    ...
    try {
      ...
      const universe = await fetchCandleResistance(consensusUniverse);
      const currentStats = buildUniverseStats(universe);
    ...
    <div className="site-shell">
      ...
      {view === "workflow" && (
        <main className="page-content subpage">
          ...
          <section className="workflow-layout">
            ...
            <aside className="bento-card node-inspector">
              ...
              <dl>
                ...
                <div><dt>후보 풀</dt><dd>{(selectedNode.id === "weekly10" && !weeklyDataReady) || (selectedNode.id === "consensusUp" && !consensusDataReady) || (selectedNode.id === "candleResistance" && !candleResistanceDataReady) ? "새로받기 또는 검색 필요" : universeStocks.length ? `${nodeCandidateCounts[selectedNode.id].toLocaleString("ko-KR")}개` : "가져오기 전"}</dd></div>
              ...
              {intersectionResults.length > 0 ? <div className="intersection-grid">
                {intersectionResults.map((result) => (
                  <button type="button" key={result.symbol} className="intersection-result" onClick={() => analyzeWorkflowStock(result)} aria-label={`${result.name} 종목분석 열기`}>
                    ...
                    <div className="result-data-tags"><span className="market-cap">{marketCapLabel(result.marketCap)}</span><span className="market-cap">{tradingValueLabel(result.tradingValue)}</span>{result.weeklyChange != null && <span className="market-cap">전주 금요일 대비 {result.weeklyChange > 0 ? "+" : ""}{result.weeklyChange.toFixed(2)}%</span>}{result.consensusUpgrade && <span className="market-cap">목표가 상향 {result.consensusReportCount}건</span>}{result.candleResistanceBreakout && <><span className="market-cap">저항 {result.candleResistance == null ? "-" : won(result.candleResistance)}</span><span className="market-cap">저항대비 +{result.candleResistanceDistance?.toFixed(2)}%</span><span className="market-cap">ADX(11) {result.candleResistanceAdx11?.toFixed(1)}</span>{result.candleResistanceDate && <span className="market-cap">저항봉 {candleDateLabel(result.candleResistanceDate)}</span>}</>}</div>
          ...
          <p className="disclaimer">종목 가져오기 노드는 NAVER 금융의 KOSPI·KOSDAQ 상장 종목에서 ETF·ETN을 제외하고 최종 시가총액 3,000억원 이상만 불러옵니다. 캔들볼륨 저항선돌파 노드는 최근 90봉 음봉의 가격·거래량 저항선과 Wilder ADX(11)를 실제 일봉으로 계산합니다. 컨센서스상향 노드는 로컬 증권리포트의 최근 5거래일 목표가상향 파일에서 종목명·코드만 추출한 공개용 스냅샷을 사용합니다. 모든 조건 노드는 이 전체 유니버스를 대상으로 계산하며, 선택한 조건의 교집합만 표시합니다. 투자 자문이 아닙니다.</p>
        </main>
      )}
