...
from pinecone import Pinecone # 👈 Cone에서 Pinecone으로 수정 완료
import json
...
def alpha_trio_supreme():
    ...
    prompt = f"""당신은 Alpha-Trio 2.5 수석 전략 위원회입니다. 
    데이터를 해부하고 에이전트 간의 파괴적인 토론을 통해 결론을 내세요.

    [데이터]: {m} | [뉴스]: {n} | [과거 오답노트]: {memory}

    - 에이전트 A(Quant): 지표 수급의 탄력을 분석하라.
    - 에이전트 B(Dewey): 현재 상황을 역사적 변곡점과 대조하여 지식을 증류하라.
    - 에이전트 C(Beckett): 낙관론을 부정하고 발생 가능한 리스크를 비판하라.
    ...
    {{
      ...
      "Knowledge_Distilled": "오늘의 시장이 가르쳐준 단 하나의 투자 원칙",
...
if __name__ == "__main__":
    print("🚀 Alpha-Trio 2.5 가동 시작 (파인콘 패치 완료)...")
    try:
        verdict, m_data = alpha_trio_supreme()
        # 지식 증류 저장
